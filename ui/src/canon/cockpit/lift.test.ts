...
describe("liftRow", () => {
  it("computes a positive lift for a clean A/B", () => {
    const r = liftRow({ kind: "skill", name: "kyc", passed: 8, total: 10, baseline_passed: 6, baseline_total: 10 });
  ...
  it("flags negative lift", () => {
    const r = liftRow({ kind: "skill", name: "x", passed: 5, total: 10, baseline_passed: 7, baseline_total: 10 });
...
describe("groupVerdict", () => {
  it("summarizes average lift across clean-A/B skills", () => {
    const v = groupVerdict([
      { kind: "skill", name: "a", passed: 8, total: 10, baseline_passed: 6, baseline_total: 10 }, // +20
      { kind: "skill", name: "b", passed: 9, total: 10, baseline_passed: 5, baseline_total: 10 }, // +40
    ]);
...
describe("liftClass", () => {
  it("earning for positive clean-A/B lift", () => {
    const b = liftClass({ kind: "skill", name: "kyc", passed: 8, total: 10, baseline_passed: 6, baseline_total: 10 });
    expect(b.kind).toBe("earning");
    expect(b.text).toBe("+20 earning");
  });
  it("not-earning for zero or negative lift", () => {
    expect(liftClass({ kind: "skill", name: "x", passed: 5, total: 10, baseline_passed: 7, baseline_total: 10 }).kind).toBe("not-earning");
    expect(liftClass({ kind: "skill", name: "x", passed: 5, total: 10, baseline_passed: 7, baseline_total: 10 }).text).toBe("-20 not earning");
    expect(liftClass({ kind: "skill", name: "y", passed: 6, total: 10, baseline_passed: 6, baseline_total: 10 }).kind).toBe("not-earning"); // 0 lift
  });
  it("unmeasured when the A/B is incomplete", () => {
    expect(liftClass({ kind: "skill", name: "x", passed: 8, total: 10, baseline_passed: 0, baseline_total: 0 }).kind).toBe("unmeasured");
    expect(liftClass({ kind: "skill", name: "x", passed: 8, total: 10, baseline_passed: 0, baseline_total: 0 }).text).toBe("no baseline");
  });
});
...
describe("groupVerdict names prune candidates", () => {
  it("names the ≤0-lift skills", () => {
    const v = groupVerdict([
      { kind: "skill", name: "kyc", passed: 8, total: 10, baseline_passed: 6, baseline_total: 10 },   // +20
      { kind: "skill", name: "legacy-x", passed: 5, total: 10, baseline_passed: 7, baseline_total: 10 }, // -20
    ]);
