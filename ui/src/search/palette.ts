...
import { structureSearch, type SearchHit } from "../api";
...
export class GlobalSearchPalette {
    ...
    card.innerHTML = `
      <header class="global-search-header">
        ...
        <input
          ...
          placeholder="Search in ${escapeHtml(shortenCwd(this.currentCwd ?? ""))}…"
          autocomplete="off"
          spellcheck="false"
        />
  ...
  private async runSearch(): Promise<void> {
    ...
    try {
      results = await structureSearch(cwd, query, HIT_LIMIT);
    } catch (err) {
      // eslint-disable-next-line no-console
      console.error("structure_search failed", err);
    ...
    if (ticket !== this.fetchTicket) return; // a newer query has won
    this.hits = results;
    this.cursor = 0;
    if (results.length === 0) {
    ...
    this.statusEl.textContent =
      results.length >= HIT_LIMIT ? `${HIT_LIMIT}+` : String(results.length);
  }
  ...
  private renderResults(query: string): void {
    ...
    void query; // (currently unused — reserved for future per-line query echo)
