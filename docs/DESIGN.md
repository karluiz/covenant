...
10. **Exit with `esc`, not an ×.** ANY dismissible surface with a close affordance in its header — full-screen / immersive surfaces (cockpit, miner, create surfaces, readers) **and modal dialogs / pickers** (Set spec, Set operator, …) — closes via a labelled `esc` affordance **and** the Escape key — never a close ×. The canonical markup is a bare button wrapping the shared pill: `<button class="…-close" aria-label="Close (Esc)"><kbd class="settings-esc">esc</kbd></button>`; the button itself is transparent/borderless and hover styles target `.settings-esc` (see `.release-close` in `styles.css`). Never render `Icons.x` as a close button. Rail/sidebar panels close via their rail toggle, so they carry no × either. **`esc` closes the whole surface back to the terminal** — never to an intermediate view (a fullscreen "board"/"cockpit" expanded from a rail closes the rail too, matching Settings / Changes / Tasker / Release log).
