...
10. **Rails carry no close button; cockpit/modal close with `esc`.** The header close affordance depends on how the surface is presented, and every dismissible surface also honours the Escape key.
    - **Rail / sidebar panels** (Tasker list, Somnus, Beacon, Project, Resources, …) carry **no × and no close button in the header** — the rail is toggled shut by its own titlebar/sidebar icon (pressing it again) or the rail toggle. The X in a rail is redundant with that toggle; never render `Icons.x` as a rail close button. Header `.rail-actions` hold only real actions (refresh, expand, new, sort).
    - **Full-screen / immersive surfaces** (cockpit, miner, create surfaces, readers, board/expanded views) **and modal dialogs / pickers** (Set spec, Set operator, …) close with the labelled `esc` pill: `<button class="…-close" aria-label="Close (Esc)"><kbd class="settings-esc">esc</kbd></button>` — transparent/borderless button, hover styles target `.settings-esc` (see `.release-close` in `styles.css`).
    - A panel that has **both** modes shows the `esc` pill **only** in fullscreen (hide it in the rail via CSS): e.g. Tasker board, Somnus expanded. **`esc` closes the whole surface back to the terminal** — never to an intermediate view (a fullscreen "board"/"cockpit" expanded from a rail closes the rail too).
...
14. **One select, everywhere.** Every dropdown is `CustomSelect` (`ui/src/ui/select.ts`) or, when its behavior genuinely doesn't fit (multi-select, rich rows), a bespoke popper that **reuses the `.ui-select__*` classes** for its surface and rows — never a from-scratch dropdown skin and never a styled native `<select>` popover. That means: sharp corners (`border-radius: 0`), body-portaled `.ui-select__popover` surface, `.ui-select__option` rows, selection check in `--accent` via `.ui-select__option-check` (no ad-hoc greens), tokenized colors so light mode comes for free. Extras (counts, avatars, group heads) layer on as additional classes beside the shared ones.
