...
fn directives_in_window_filters_by_proposed_ms_asc() {
    let m = Memory::open_in_memory().unwrap();
    m.log_directive("d1", 100, "proposed", "Stop", "x", "r", None)
        .unwrap();
    m.log_directive("d2", 300, "approved", "Stop", "y", "r", None)
        .unwrap();
    m.log_directive("d3", 200, "rejected", "Stop", "z", "r", Some("nope"))
        .unwrap();
    m.log_directive("d4", 50, "proposed", "Stop", "old", "r", None)
        .unwrap();
    ...
    assert_eq!(
        ids,
        vec!["d1", "d3", "d2"],
        "ASC by proposed_ms, lower bound inclusive"
    );
}
...
async fn summary_cache_hit_skips_llm_when_few_events() {
    ...
    m.write_summary(NOW - 1000, "operator built feature X", 0, 10, 5)
        .unwrap();
    // Append a few events (well below the 50-event staleness threshold).
    for i in 0..5 {
        m.append_event(NOW - 500 + i, "BlockFinished", "S", "{}")
            .unwrap();
    }
    let agent = ChatAgent {
        memory: &m,
...
async fn summary_cache_miss_calls_llm_with_scoped_payload() {
    ...
    m.write_summary(NOW - 10_000, "pre-AOM state", 0, 10, 5)
        .unwrap();
    ...
    assert!(
        sys.contains("rolling 24h"),
        "system must mention scope label"
    );
    assert!(
        sys.contains("language"),
        "system must instruct language detection"
    );
    assert!(sys.contains("Do NOT propose directives"));
    assert!(
        user.contains("pre-AOM state"),
        "user payload must include rolling"
    );
    assert!(
        user.contains("halt deploy"),
        "user payload must include directive"
    );
...
async fn summary_frozen_mode_skips_llm_and_appends_disclaimer() {
    ...
    assert!(
        turn.assistant_text.contains("modo congelado"),
        "frozen reply must include disclaimer; got: {}",
        turn.assistant_text
    );
}
...
async fn summary_never_returns_directive() {
    ...
    let turn = agent.turn(NOW, "/summary").await.unwrap();
    assert!(
        turn.proposed_directive.is_none(),
