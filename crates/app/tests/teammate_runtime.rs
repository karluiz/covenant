...
fn session() -> SessionId { SessionId::new() }
...
fn pinned_to_ontask_unpins_first() {
    ...
    let t = TaskId::new();
    rt.start_task(id, t, None).expect("pinned → ontask is legal (auto-unpin)");
    assert!(matches!(rt.state(id), Some(OperatorState::OnTask { task, session: None }) if task == t));
}
