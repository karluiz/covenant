...
pub fn classify_command(cmd: &str) -> Option<BuildKind> {
    ...
    if c.contains("clippy")
        || c.contains("eslint")
        || c.contains("ruff")
        || c.contains("npm run lint")
        || c.contains("yarn lint")
    {
    ...
    if c.contains("cargo build")
        ...
        || c.contains("make build")
        || c.contains("go build")
        || c.contains("cargo check")
        || c.contains("yarn build")
    {
...
mod tests {
    ...
    fn ignores_unrelated_commands() {
        ...
        assert_eq!(
            classify_command("echo cargo build is great"),
            Some(BuildKind::Build)
        ); // documented over-match
    }
}
